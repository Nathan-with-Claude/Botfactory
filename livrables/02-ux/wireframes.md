...
### Écran W-09 : Panneau Broadcast (interface superviseur web)

**Persona** : Karim B. (Superviseur logistique terrain)
**Objectif** : Envoyer en moins de 3 clics un *message broadcast* à tous ses livreurs
actifs ou à un secteur ciblé, et consulter l'historique des *broadcasts* du jour avec
le statut "vu" par livreur
**URL/Route** : panneau latéral (drawer) ouvert par-dessus W-01 — route /supervision/broadcast
**Domain Events déclenchés** : BroadcastEnvoyé
**Domain Events affichés** : BroadcastLu (via Read Model côté superviseur)

#### Layout — Panneau ouvert (drawer latéral droit)

```
┌──────────────────────────────────────────────────────────────────────┐
│ DocuPost | [● LIVE]                     [sync][notif]| Karim B.     │  TopAppBar (inchangée)
├──────────┬───────────────────────────────────────────────────────────┤
│          │                          ┌──────────────────────────────┐ │
│  [ ] Pré │                          │ Broadcast                [✕] │ │  Drawer latéral droit
│  paration│    W-01 (atténué)        ├──────────────────────────────┤ │  largeur : 420px
│  [■] Sup │                          │ Type de message              │ │  fond blanc, shadow-xl
│  ervision│                          │ (•) Alerte                   │ │  séparateur gauche 1px
│  [►] Liv │                          │ ( ) Info                     │ │  outline-variant
│    reurs │                          │ ( ) Consigne                 │ │
│          │                          ├──────────────────────────────┤ │
│  ────────│                          │ Destinataires                │ │
│  [⊕] Bro │                          │ (•) Tous les livreurs actifs │ │  Icône broadcast dans SideNavBar
│  adcast  │                          │ ( ) Secteur 1                │ │  libellé "Broadcast"
│  ────────│                          │ ( ) Secteur 2                │ │
│  [?] Aide│                          │ ( ) Secteur 3                │ │
│  [⏻] Déco│                          │ → 18 livreurs concernés      │ │  Compteur dynamique livreurs
│          │                          ├──────────────────────────────┤ │
│          │                          │ Message                      │ │
│          │                          │ ┌──────────────────────────┐ │ │
│          │                          │ │                          │ │ │  Zone de saisie texte libre
│          │                          │ │ Saisissez votre message  │ │ │  multiline — max 280 car.
│          │                          │ │                          │ │ │
│          │                          │ └──────────────────────────┘ │ │
│          │                          │                    0 / 280   │ │  Compteur de caractères
│          │                          ├──────────────────────────────┤ │
│          │                          │ ┌──────────────────────────┐ │ │
│          │                          │ │       ENVOYER            │ │ │  Bouton primaire (#0037B0)
│          │                          │ └──────────────────────────┘ │ │  désactivé si texte vide
│          │                          ├──────────────────────────────┤ │
│          │                          │ Envoyés aujourd'hui (3)      │ │  Historique du jour
│          │                          ├──────────────────────────────┤ │
│          │                          │ [ALERTE] 09:14               │ │  Item historique — badge type
│          │                          │ Rue du Port barrée — secteur │ │  texte tronqué 1 ligne
│          │                          │ 2 concerné                   │ │
│          │                          │ Vu par 5 / 6 livreurs  [>]   │ │  Statut vu + chevron détail
│          │                          ├──────────────────────────────┤ │
│          │                          │ [INFO] 11:32                 │ │
│          │                          │ Dépôt ferme à 17h auj.       │ │
│          │                          │ Vu par 18 / 18 livreurs [>]  │ │
│          │                          ├──────────────────────────────┤ │
│          │                          │ [CONSIGNE] 14:07             │ │
│          │                          │ Ne pas livrer en sous-sol     │ │
│          │                          │ intempéries — météo France    │ │
│          │                          │ Vu par 12 / 18 livreurs [>]  │ │
│          │                          └──────────────────────────────┘ │
└──────────┴───────────────────────────────────────────────────────────┘
```

#### Toast de confirmation (après ENVOYER)

```
┌──────────────────────────────────────────────────────────────────────┐
│  ✓ Message envoyé à 18 livreurs                          [Fermer]   │  Toast fond emerald-600
└──────────────────────────────────────────────────────────────────────┘
```

#### Composants

**Entrée SideNavBar "Broadcast" :**
- Icône : `campaign` (Material Symbols).
- Libellé : "Broadcast".
- Positionnée sous "Livreurs" dans la SideNavBar, avant "Aide".
- Badge rouge optionnel si aucun *broadcast* envoyé depuis plus de X heures
  (règle métier à affiner avec le PO).
- Clic : ouvre le drawer W-09 par-dessus W-01 sans naviguer hors de la page de supervision.

**Sélecteur de type de message :**
- 3 options radio : Alerte, Info, Consigne.
- Couleur du badge selon le type :
  - Alerte : fond error-container (#FFDAD6), texte error (#BA1A1A).
  - Info : fond primary-container (#E8F0FE), texte primary (#0037B0).
  - Consigne : fond secondary-container, texte secondary.
- Sélection obligatoire avant activation du bouton ENVOYER.

**Sélecteur de destinataires :**
- 4 options radio : Tous les livreurs actifs / Secteur 1 / Secteur 2 / Secteur 3.
- Compteur dynamique sous les options : "→ N livreurs concernés" (calculé selon la sélection
  et le référentiel des livreurs actifs du jour).
- Libellé "Tous les livreurs actifs" : exclut automatiquement les livreurs hors connexion
  ou dont la *tournée* est clôturée.

**Zone de saisie du message :**
- Champ texte multiline, placeholder "Saisissez votre message".
- Compteur de caractères : "N / 280" en bas à droite, passe en orange (#7F2500) à partir
  de 250 caractères, en rouge (#BA1A1A) au-delà de 280 (blocage de saisie).
- Pas de formatage riche (texte brut uniquement) — cohérent avec la contrainte SMS-like
  exprimée par Karim.

**Bouton ENVOYER :**
- Désactivé (grisé) si type non sélectionné ou texte vide.
- Actif : fond primary (#0037B0), texte blanc, shadow-md.
- Après envoi : toast de confirmation emerald-600 en haut de page "Message envoyé à N
  livreurs." Disparition automatique après 5 secondes.

**Historique des broadcasts du jour :**
- Liste compacte chronologique inverse (le plus récent en haut).
- Chaque item : badge type coloré + heure d'envoi + texte tronqué 1 ligne + statut
  "Vu par N / M livreurs" + chevron [>] vers le détail.
- Le détail (accès via [>]) affiche la liste nominative des livreurs avec statut vu / non vu.
- Aucun *broadcast* n'est modifiable ou supprimable après envoi (traçabilité).

#### Interactions principales

- Clic sur "Broadcast" dans SideNavBar : ouverture du drawer W-09. W-01 reste visible et
  actif en arrière-plan (atténué par un overlay semi-transparent). Max 1 clic depuis W-01.
- Sélection du type : badge type affiché en aperçu. Bouton ENVOYER reste inactif tant que
  le texte est vide.
- Sélection du ciblage : compteur de livreurs mis à jour instantanément.
- Saisie du message : compteur de caractères mis à jour en temps réel.
- Clic ENVOYER : envoi via FCM. Toast de confirmation. Item ajouté en tête de l'historique
  avec statut "Vu par 0 / N livreurs". Le formulaire est réinitialisé. Le drawer reste ouvert.
- Clic [>] sur un item historique : expansion inline ou modal de la liste nominative des
  livreurs avec statut "vu" / "non vu" et horodatage de lecture.
- Clic [✕] ou clic hors du drawer : fermeture du panneau, retour à W-01.

#### États spéciaux

- Aucun livreur actif (secteur sélectionné vide) : "Aucun livreur actif dans ce secteur.
  Choisissez un autre ciblage." Bouton ENVOYER désactivé.
- Erreur d'envoi (réseau) : toast rouge "Envoi impossible. Vérifiez votre connexion.
  Réessayez." Bouton ENVOYER réactivé.
- Historique vide (aucun *broadcast* du jour) : "Aucun message envoyé aujourd'hui."
- Mode offline superviseur : le bouton ENVOYER est désactivé avec message d'avertissement
  "Connexion indisponible — envoi impossible en mode hors ligne."

**Termes du domaine annotés** : *broadcast*, *message broadcast*, *alerte*, *info*,
*consigne*, *ciblage*, *secteur*, *livreurs actifs*, *statut vu*, *historique broadcast*

---

### Écran M-08 : Zone messages broadcast (app mobile livreur)

**Persona** : Pierre Morel (Livreur terrain)
**Objectif** : Consulter les *messages broadcast* reçus du superviseur dans la journée,
ne jamais en manquer un, et être alerté en temps réel à réception
**Route** : /tournee/broadcast
**Domain Events affichés** : BroadcastLu (émis à l'ouverture de l'écran ou à la
visualisation du message)
**Domain Events déclenchés** : BroadcastLu (accusé de réception envoyé au superviseur)

#### Layout — Accès depuis M-02 (badge et bouton dédié)

```
┌─────────────────────────────────────┐
│  [Menu]  Tournée du 21/04/2026  [?] │  Header M-02
│                        [🔔 2]       │  Icône broadcast avec badge rouge "2"
├─────────────────────────────────────┤  (messages non lus)
│  Reste à livrer : 14 / 22 colis     │
│  ...                                │  Contenu M-02 habituel
└─────────────────────────────────────┘
```

L'icône `campaign` (broadcast) dans le header de M-02 remplace ou complète l'icône
"Mes consignes". Badge rouge avec le nombre de *messages broadcast* non lus.

#### Layout — Écran M-08 (liste des messages)

```
┌─────────────────────────────────────┐
│  [<]  Messages superviseur          │  Header — bouton retour M-02
│                                     │
│  Aujourd'hui — 3 messages           │  Résumé du jour
├─────────────────────────────────────┤
│  ┌───────────────────────────────┐  │
│  │ [ALERTE]               09:14  │  │  Badge type coloré + heure
│  │ Rue du Port barrée —          │  │  Texte du message (complet)
│  │ secteur 2 concerné.           │  │
│  │ De : Karim B.                 │  │  Émetteur identifié
│  └───────────────────────────────┘  │  Fond error-container/30 (non lu → blanc après lecture)
│  ┌───────────────────────────────┐  │
│  │ [INFO]                 11:32  │  │
│  │ Le dépôt ferme à 17h          │  │
│  │ aujourd'hui. Rentrez avant.   │  │
│  │ De : Karim B.                 │  │
│  └───────────────────────────────┘  │  Fond blanc (déjà lu)
│  ┌───────────────────────────────┐  │
│  │ [CONSIGNE]             14:07  │  │
│  │ Ne pas livrer en sous-sol —   │  │
│  │ intempéries en cours.         │  │
│  │ De : Karim B.                 │  │
│  └───────────────────────────────┘  │
└─────────────────────────────────────┘
```

#### Layout — Overlay broadcast reçu (affiché sur l'écran courant)

```
┌─────────────────────────────────────┐
│  ┌─────────────────────────────┐   │
│  │  [ALERTE] SUPERVISEUR       │   │  Bandeau overlay — couleur selon type
│  │  Rue du Port barrée —       │   │  Alerte : fond error (#BA1A1A), texte blanc
│  │  secteur 2 concerné.        │   │  Info : fond primary (#0037B0), texte blanc
│  │              [VOIR] [✕]     │   │  Consigne : fond secondary, texte blanc
│  └─────────────────────────────┘   │
└─────────────────────────────────────┘
```

Cet overlay est cohérent avec M-06 (overlay d'*instruction*). Il s'affiche par-dessus
l'écran courant (M-02, M-03, M-04, M-05) à réception du *broadcast* via FCM,
y compris si l'app est en arrière-plan (notification push système).

#### Composants

**Badge compteur sur l'icône broadcast dans M-02 :**
- Icône `campaign` dans le header de M-02.
- Badge rouge arrondi avec le nombre de *messages broadcast* non lus (même style que
  le badge "Mes consignes").
- Badge disparaît quand tous les messages du jour ont été ouverts.

**Liste des messages dans M-08 :**
- Ordre chronologique inverse (le plus récent en haut).
- Chaque item :
  - Badge type : ALERTE (fond error-container, texte error), INFO (fond primary-container,
    texte primary), CONSIGNE (fond secondary-container, texte secondary).
  - Heure d'envoi (format HH:mm si du jour, JJ/MM HH:mm sinon).
  - Texte complet du message (non tronqué — max 280 caractères).
  - Émetteur : "De : [Prénom Nom du superviseur]".
  - Fond de l'item : coloré (selon type/30 opacité) si non lu, blanc si déjà lu.
- À l'ouverture de M-08 : tous les messages visibles passent à l'état "lu". L'événement
  *BroadcastLu* est émis pour chaque message non encore acquitté.

**Overlay de réception (push) :**
- Bandeau en haut de l'écran courant, couleur selon le type (Alerte = rouge, Info = bleu,
  Consigne = secondary).
- Badge "[ALERTE]" / "[INFO]" / "[CONSIGNE]" + libellé "SUPERVISEUR".
- Texte du message (2 lignes max, tronqué si nécessaire).
- Bouton "VOIR" : navigue vers M-08 et positionne sur ce message.
- Bouton [✕] : ferme l'overlay. Le message reste non lu dans M-08 jusqu'à ouverture.
- Disparition automatique après 15 secondes si aucune interaction (durée plus longue
  que M-06 car message informatif, pas une instruction).

#### Règles de statut des messages broadcast

| Statut  | Déclencheur                                                              | Domain Event  |
|---------|--------------------------------------------------------------------------|---------------|
| Non lu  | Réception de la notification push FCM (background ou foreground)         | —             |
| Lu      | Ouverture de M-08 et message visible dans la liste (lecture confirmée)   | BroadcastLu   |

Le statut "lu" est un acquittement côté livreur qui met à jour le compteur "Vu par N / M"
visible dans W-09 (panneau superviseur). La communication reste unidirectionnelle :
Pierre ne peut pas répondre au message.

#### Interactions principales

- Appui sur l'icône broadcast dans M-02 : navigation vers M-08.
- Ouverture de M-08 : tous les messages non lus passent à lu. Badge réinitialisé.
- Appui sur "VOIR" dans l'overlay : navigation vers M-08, scroll vers le message concerné.
- Appui sur [✕] dans l'overlay : fermeture sans marquer le message comme lu.
- Bouton retour [<] : retour vers M-02. Badge mis à jour.

#### États spéciaux

- Aucun message du jour : "Votre superviseur n'a pas envoyé de message aujourd'hui."
- Mode offline : les messages reçus avant la perte de connexion restent accessibles
  (stockage local). Les nouveaux broadcasts ne peuvent pas arriver en mode offline
  (FCM indisponible) — bandeau orange "Hors connexion" en tête de M-08.
- Application en arrière-plan : la notification push système Android s'affiche dans la
  barre de notifications. À la réouverture de l'app, l'overlay est affiché si le message
  date de moins de 5 minutes.

**Termes du domaine annotés** : *broadcast*, *message broadcast*, *alerte*, *info*,
*consigne*, *statut vu*, *notification push*, *livreur*, *superviseur*
...
| M-08  | Mobile    | Livreur     | P7 — Broadcast       | BroadcastLu                                  |
...
| W-08  | Web       | Superviseur | P6 — État livreurs   | AffectationEnregistree, TourneeLancee (Read Model VueLivreur) |
| W-09  | Web       | Superviseur | P7 — Broadcast       | BroadcastEnvoyé, BroadcastLu (compteur)      |
