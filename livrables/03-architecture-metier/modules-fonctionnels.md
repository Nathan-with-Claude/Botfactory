...
>
> Mise à jour 1.1 (2026-03-20) : ajout du Module 8 — Planification de Tournée (BC-07)
> suite à l'entretien complémentaire avec M. Renaud. Ce module est un prérequis bloquant
> pour le Module 1 (Orchestration de Tournée).
...
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │              MODULE 8 — Planification de Tournée                │   │
│  │  [Core Domain]  PlanDuJour · TournéeTMS · Affectation · Véhicule│   │
│  └────────────────────────┬────────────────────────────────────────┘   │
│           TournéeLancée    │                                             │
│                            ▼                                             │
│  ┌─────────────────────────────────────────────────────────────────┐   │
...
|---|---|---|
| ChargerTournee(livreurId, date) | Authentification livreur | Livreur authentifié, tournée assignée (lancée via Module 8) |
...
TournéeLancée (depuis Module 8 — Planification de Tournée)

### Interactions avec autres modules

| Module cible | Type | Mécanisme | Données échangées |
|---|---|---|---|
| Module 8 — Planification | Event (consommateur) | TournéeLancée → création Tournée terrain | tourneeTMSId, livreurId, vehiculeId |
...
5. Une Tournée ne peut être chargée que si elle a été lancée depuis le Module 8
   (événement TournéeLancée reçu).
...
Gérer l'import des tournées depuis le TMS, la vérification de leur composition, l'affectation
livreur/véhicule et le lancement des tournées. Ce module est un prérequis bloquant pour le
Module 1 : sans une tournée lancée par ce module, aucun livreur n'a de tournée disponible
dans l'application mobile.

"Chaque matin, je dois vérifier que chaque tournée est correcte avant de lancer les
livreurs." (M. Renaud)
...
| Élément | Type DDD | Description |
|---|---|---|
| PlanDuJour | Aggregate Root | Ensemble des TournéesTMS pour une journée, géré par le responsable logistique |
| TournéeTMS | Entity (dans PlanDuJour) | Tournée brute importée depuis le TMS, en attente d'affectation |
| Affectation | Entity | Association d'un livreur et d'un véhicule à une TournéeTMS |
| Véhicule | Entity | Ressource transport assignable (immatriculation, type, capacité) |
| ColisImporté | Value Object | Colis brut TMS avec référence, adresse et contraintes |
| ContrainteHoraire | Value Object | Plage horaire de livraison imposée par le TMS |
| CompositionDeTournée | Value Object | Ensemble des colis, zones et contraintes d'une TournéeTMS |
| StatutTournéeTMS | Value Object | NonAffectée / Affectée / Lancée |

### Capacités couvertes

- 0.1 Import TMS (réception et parsing des tournées du TMS)
- 0.2 Vérification de composition (zones, colis, contraintes horaires)
- 0.3 Affectation livreur / véhicule
- 0.4 Lancement des tournées (rend la tournée visible au livreur)

### Composants internes

| Composant | Rôle |
|---|---|
| ImporteurTMS | Adaptateur ACL : appelle l'API TMS (ou reçoit le batch), parse et normalise les données vers le modèle TournéeTMS DocuPost |
| VérificateurComposition | Valide les règles de composition métier : présence de colis, zones géographiques cohérentes, contraintes horaires non conflictuelles |
| GestionnaireAffectation | Gère la création et la validation des Affectations livreur/véhicule, contrôle les unicités journalières |
| LanceurTournée | Valide l'Affectation complète et publie l'événement TournéeLancée vers BC-01 |

### Commandes exposées (API REST)

| Commande | Endpoint | Déclencheur | Préconditions |
|---|---|---|---|
| ImporterTourneesTMS(date) | POST /plans/{date}/import | Planificateur ou appel manuel | TMS disponible, date valide |
| VerifierComposition(tourneeTMSId) | POST /tournees/{id}/verifier | Logisticien valide composition | TournéeTMS au statut NonAffectée |
| CreerAffectation(tourneeTMSId, livreurId, vehiculeId) | POST /affectations | Logisticien sélectionne livreur et véhicule | TournéeTMS NonAffectée, livreur et véhicule disponibles |
| LancerTournee(tourneeTMSId) | POST /tournees/{id}/lancer | Logisticien valide le lancement | Affectation complète (livreurId + vehiculeId renseignés) |

### Queries exposées

| Query | Endpoint | Réponse |
|---|---|---|
| GetPlanDuJour(date) | GET /plans/{date} | PlanDuJour avec liste des TournéesTMS et leur statut |
| GetTourneesAvecStatutAffectation(date) | GET /plans/{date}/tournees | Liste des TournéesTMS avec statut NonAffectée / Affectée / Lancée |

### Domain Events émis

| Événement | Déclencheur | Consommateurs |
|---|---|---|
| TournéeImportéeTMS | Import depuis TMS finalisé | Tableau de bord logisticien (Module 3 étendu ou vue dédiée) |
| CompositionVérifiée | Logisticien valide la composition d'une TournéeTMS | — (état interne) |
| AffectationEnregistrée | Logisticien affecte livreur + véhicule | — (état interne PlanDuJour) |
| TournéeLancée | Logisticien lance la tournée | Module 1 — Orchestration de Tournée |

### Domain Events consommés

Aucun. Ce module est l'initiateur de la chaîne : il n'a pas de dépendance événementielle
en entrée.
...
| Module cible | Type | Mécanisme | Données échangées |
|---|---|---|---|
| Module 1 — Orchestration | Event (émetteur) | TournéeLancée → création Tournée terrain | tourneeTMSId, livreurId, vehiculeId, planId, horodatage |
| Module 6 — Identité | Query (consommateur) | Token JWT → identité logisticien + référentiel livreurs | superviseurId, livreurId, rôle |
| TMS Externe | API REST (consommateur via ACL) | Import API ou batch planifié | Données brutes TMS (format propriétaire) |

### Règles métier critiques

1. Une Affectation ne peut être créée que pour une TournéeTMS au statut "NonAffectée".
2. Une TournéeTMS ne peut être lancée que si elle possède une Affectation avec livreurId
   et véhiculeId renseignés.
3. Le lancement est irréversible : il publie TournéeLancée et passe la TournéeTMS au
   statut "Lancée".
...
6. Toute Affectation est historisée avec l'identité du superviseur qui l'a validée.
...
|  | M1 Orchestration | M2 Preuves | M3 Supervision | M4 Notification | M5 Intégration SI | M6 Identité | M7 Reporting | M8 Planification |
|---|---|---|---|---|---|---|---|---|
| **M1 Orchestration** | — | Events (PreuveCapturée) | Commands (ApplInstr.) | Events (InstructionReçue) | Events (tous statuts) | Queries (token) | — | Events (TournéeLancée) |
...
| **M8 Planification** | Events (TournéeLancée → M1) | — | — | — | — | Queries (token + livreurs) | — | — |
...
|---|---|---|---|
| TournéeImportéeTMS | M8 | (tableau de bord) | planId, date, nombreTournees, horodatage |
| AffectationEnregistrée | M8 | — (interne) | tourneeTMSId, livreurId, vehiculeId, superviseurId, horodatage |
| TournéeLancée | M8 | M1 | tourneeTMSId, livreurId, vehiculeId, planId, horodatage |
...
|---|---|---|---|
| ImporterTourneesTMS | Planificateur / Interface Web (logisticien) | M8 | TMS disponible, date valide |
| CreerAffectation | Interface Web (logisticien) | M8 | TournéeTMS NonAffectée, livreur + véhicule disponibles |
| LancerTournee | Interface Web (logisticien) | M8 | Affectation complète validée |
| ChargerTournee | App Mobile (livreur) | M1 | Livreur authentifié, TournéeLancée reçue depuis M8 |
...
### 3. Anti-Corruption Layer TMS (Module 8 — composant ImporteurTMS)
Le composant ImporteurTMS est le seul point d'intégration avec le TMS externe. Il isole
le modèle TournéeTMS DocuPost du format propriétaire du TMS. Aucun autre module ne
connaît le format TMS.

### 4. Offline-first (Module 1 + Module 2)
...
### 6. Identité partagée via Shared Kernel
...
### 7. Chaîne de dépendance Module 8 → Module 1
Le Module 8 est le point de départ obligatoire du cycle journalier. L'événement
TournéeLancée qu'il publie est la seule porte d'entrée permettant au Module 1
d'initialiser une Tournée terrain. Cette dépendance est intentionnelle et garantit
qu'aucune tournée ne démarre sans affectation validée.
