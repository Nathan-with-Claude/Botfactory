...
// ─── US-040 : Export CSV enrichi (Destinataire + Statut) ─────────────────────

/**
 * Statut d'un colis individuel tel que connu au moment de l'export.
 */
export type StatutColisCsv = 'LIVRE' | 'ECHEC' | 'EN_COURS';

/**
 * Ligne CSV enrichie (US-040) : #Colis, Destinataire, Adresse, Zone, Contrainte, Statut
 */
export interface ColisCSVRowEnrichi {
  numeroColis: string;
  destinataire: string;
  adresse: string;
  zone: string;
...
/**
 * Colis individuel avec données de composition enrichies (US-040).
 */
export interface ColisCompositionEnrichi {
  numeroColis: string;
  destinataire?: string;
  adresse: string;
  zone: string;
  contrainte: string;
  statut: StatutColisCsv;
}

/**
 * Extension de TourneePourExport avec la liste des colis individuels enrichis (US-040).
 */
export interface TourneePourExportEnrichie extends TourneePourExport {
  colis: ColisCompositionEnrichi[];
}

/**
 * Traduit le statut interne en libellé humain pour la colonne CSV Statut.
 */
function traductionStatut(statut: StatutColisCsv): string {
  switch (statut) {
...
/**
 * Construit les lignes CSV enrichies à partir de la liste de colis individuels.
 * Colonnes : #Colis, Destinataire, Adresse, Zone, Contrainte, Statut
 */
export function construireColisCSVRowsEnrichis(tournee: TourneePourExportEnrichie): ColisCSVRowEnrichi[] {
  return tournee.colis.map(colis => ({
    numeroColis: colis.numeroColis,
    destinataire: colis.destinataire ?? '',
    adresse: colis.adresse,
    zone: colis.zone,
    contrainte: colis.contrainte,
    statut: traductionStatut(colis.statut),
  }));
}

/**
 * Sérialise les lignes CSV enrichies en texte UTF-8 avec BOM.
 * En-tête : #Colis,Destinataire,Adresse,Zone,Contrainte,Statut
 */
export function serialiserEnCSVEnrichi(rows: ColisCSVRowEnrichi[]): string {
  const echapper = (valeur: string): string => {
    if (valeur.includes(',') || valeur.includes('"') || valeur.includes('\n')) {
      return `"${valeur.replace(/"/g, '""')}"`;
    }
  ...
  const entete = '#Colis,Destinataire,Adresse,Zone,Contrainte,Statut';
  const lignes = rows.map(row =>
    [
      echapper(row.numeroColis),
      echapper(row.destinataire),
      echapper(row.adresse),
      echapper(row.zone),
      echapper(row.contrainte),
      echapper(row.statut),
...
/**
 * Point d'entrée principal — exporte en CSV enrichi la composition d'une tournée (US-040).
 * Orchestre construireColisCSVRowsEnrichis → serialiserEnCSVEnrichi → declencherTelechargement.
 */
export function exporterCSVEnrichi(tournee: TourneePourExportEnrichie): void {
  const rows = construireColisCSVRowsEnrichis(tournee);
  const contenu = serialiserEnCSVEnrichi(rows);
  const nomFichier = construireNomFichier(tournee.codeTms, tournee.date);
  declencherTelechargement(contenu, nomFichier);
}
