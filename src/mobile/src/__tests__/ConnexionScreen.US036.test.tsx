...
describe('US-036 — SC5 : persistance préférence dans AsyncStorage', () => {
  it('persiste cardSsoOuverte = false dans AsyncStorage après repli manuel (utilisateur déjà connecté)', async () => {
    // US-043 : la persistance de cardSsoOuverte ne se fait QUE pour un utilisateur ayant
    // déjà effectué une connexion (hasConnectedOnce=true). Avant connexion, le repli
    // n'est pas mémorisé en AsyncStorage.
    mockAsyncStorage.getItem.mockImplementation(async (key: string) => {
      if (key === 'hasConnectedOnce') return 'true';
      return null; // cardSsoOuverte non encore positionnée → card repliée (dejaConnecte=true)
    });
    ...
    // La card est repliée par défaut (hasConnectedOnce=true, pas de préférence card)
    // Appui pour l'étendre
    await act(async () => {
      fireEvent.press(toggleBtn);
    });
    ...
    await waitFor(() => {
      ...
      // Le dernier appel doit être 'false' (card repliée)
      const dernierAppel = callsForCardSso[callsForCardSso.length - 1];
      expect(dernierAppel[1]).toBe('false');
    });
  });
